...
// NOTE: This is a copy of the CFBF reader in Scrubkit.Email (used there for .msg), with one
// addition: streams whose name starts with an OLE control byte (e.g. \x05SummaryInformation)
// are also indexed under the stripped name. Both binary Office and Outlook .msg sit in the same
// OLE2 container, but the two add-on packages stay independent (each references only
// Scrubkit.Abstractions), so the small reader is duplicated rather than shared.

/// <summary>
/// A minimal reader for the OLE2 / Compound File Binary Format (CFBF) — the container the
/// legacy binary Office formats (.doc/.xls/.ppt) use. Just enough to enumerate the top-level
/// streams and read their bytes: it parses the header, FAT, directory, and (for small streams)
/// the mini-FAT stream. Not a general OLE implementation; storages beyond the root are
/// flattened by name.
/// </summary>
internal sealed class CompoundFile
{
    ...
    public CompoundFile(byte[] data)
    {
        ...
        for (var e = 0; e < count; e++)
        {
            ...
            // OLE names like "\x05SummaryInformation" are also reachable without the control byte.
            if (name.Length > 1 && name[0] < ' ') _streams[name.Substring(1)] = bytes;
        }
    }
